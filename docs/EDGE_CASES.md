...
|------------|---------|--------------------|-------------------------|
| **Empty / whitespace** | `""`, `"   "` | Short-circuits to a safe fallback (General/Other, Medium, Customer Support), `needs_human_review: true`, `error: "empty_input"`, `engine: "fallback"`. **No model call.** | Nothing to classify — spending a model call would be wasteful and could hallucinate. Escalate to a human instead. |
