...
| **Unknown key / wrong type in model output** | `{"category": "...", "surprise": 1}` | `TriageResult` uses `extra="forbid"` → `ValidationError` → retry + corrective repair → safe fallback. Never stored raw. | Strict JSON: a stray field or a self-contradiction (is_ticket vs routing) is treated as a bad response, not silently accepted. |
